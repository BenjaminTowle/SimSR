...
def load_data(
    ...
    contexts = [special_token.join(C) for C in contexts]
    persona = [special_token.join(P) for P in persona]
    ...
    inputs = {}
    #inputs["input_ids"] = tokenizer(contexts, padding="max_length", max_length=max_context_length, truncation=True).input_ids
...
def main():
    ...
    with open("../../../data/personachat/train_both_original.txt") as f:
        lines = f.readlines()
        a = 2
