...
class BiencoderMixin(abc.ABC):

    @abc.abstractmethod
    def _get_embedding(self, *args, **kwargs) -> torch.Tensor:
        pass
...
class Biencoder1D(BaseModel):
    def __init__(self, config):
        super().__init__(config)
        self.pad = 0
        self.proj = nn.Linear(self.config.hidden_size, 1)
    ...
    def _get_embedding(self, input_ids, attention_mask):
        embeds = self.bert(input_ids, attention_mask=attention_mask).last_hidden_state[:, 0, :]
        embeds = self.proj(embeds) #* BERT_SCALING_FACTOR
        return embeds
    ...
    def forward(
        ...
        labels = torch.arange(bsz, device=device) if labels is None else labels
        loss = nn.CrossEntropyLoss()(scores, labels)
...
class PseudoRelevanceFeedback(BaseModel):
    def __init__(self, config):
        super().__init__(config)
        self.pad = 0
        self.dataset = None
